...
1. 修改人设与回复逻辑，指示智能体使用**谷歌搜索**插件来回答自己不确定的问题。否则，智能体可能不会按照预期调用该工具，在左侧提示词侧输入“{”就会让你选择插件
...
由于安装时，是添加到`<body>`中，而大多博客都没办法直接嵌入，所以编写一个 `vue组件` 通过`#layout-top`插槽传递，如果想要拖拽，不超出视口等功能可以查看[GitHub](https://github.com/liyao52033/vitepress-teek/blob/docs/docs/.vitepress/theme/components/Coze/index.vue)
  ...
  externalScript.onload = () => {
    if (window.CozeWebSDK?.WebChatClient) {
      new window.CozeWebSDK.WebChatClient({
        config: {
          bot_id: secureInfo.botId, // [!!code highlight]
        },
        componentProps: {
        ...
        auth: {
          type: "token",
          token: secureInfo.botAccessToken, // [!!code highlight]
          onRefreshToken: () => secureInfo.botAccessToken, // [!!code highlight]
        },
      });
    } else {
