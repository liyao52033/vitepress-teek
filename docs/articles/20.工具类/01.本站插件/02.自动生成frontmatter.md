...
- 插件默认给 Markdown 文件生成 title 和 date 两个属性，其中 title 为文件名（支持带序号的文件名，如 01.xx.md），date 为文件的创建日期，permalinkPrefix和categories选项都是可选的，如果想拓展 frontmatter 的内容，则使用 `transform` 函数。
