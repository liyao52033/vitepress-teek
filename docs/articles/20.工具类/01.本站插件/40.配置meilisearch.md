...
<template>
    <div>
        <!-- 搜索触发按钮 -->
        <button type="button" class="DocSearch DocSearch-Button" aria-label="Search" @click="toggleSearch">
        ...
        <!-- 搜索弹窗 -->
        <div v-if="isSearchOpen" class="DocSearch DocSearch-Container" role="dialog" aria-modal="true"
             aria-labelledby="docsearch-label" @click="closeSearch">
            <div class="DocSearch-Modal" @click.stop>
                <header class="DocSearch-SearchBar">
                    <form class="DocSearch-Form" @submit.prevent>
                        ...
                        <input id="docsearch-input" v-model="searchQuery" @input="handleInput" ref="searchInput"
                               class="DocSearch-Input" type="search"
                               :placeholder="meiliConfig.placeholder || '搜索文档...'"
                               spellcheck="false" autocomplete="off" autocorrect="off" autocapitalize="off" autofocus
                               @keydown.down.prevent="moveDown" @keydown.up.prevent="moveUp"
                               @keydown.esc.prevent="closeSearch" @keydown.enter.prevent="goToHit" />
                        <button type="reset" class="DocSearch-Reset" v-if="searchQuery" @click="clearSearch">✕</button>
                    </form>
                    <button type="button" class="DocSearch-Cancel" @click="closeSearch">Cancel</button>
                </header>

                <!-- 搜索结果区域 -->
                <div class="DocSearch-Dropdown">
                    <div class="DocSearch-Dropdown-Container" ref="hitsContainer">
                        <!-- 分组渲染搜索结果 -->
                        <section class="DocSearch-Hits" v-for="(group, groupIdx) in groupedHits" :key="groupIdx"
                                 v-if="searchQuery">
                            <div class="DocSearch-Hit-source">{{ group.title }}</div>
                            <ul role="listbox" :id="'docsearch-hits' + groupIdx + '-list'">
                                <!-- 列表项：添加active类模拟默认hover，使用highlightKeyword处理关键词高亮 -->
                                <li v-for="(hit, itemIdx) in group.items" :key="itemIdx" class="DocSearch-Hit"
                ...
                <footer class="DocSearch-Footer"
                    ...
                    <div class="DocSearch-Logo" style="display: flex; align-items: center;">
                        ...
                        <a href="https://www.meilisearch.com" target="_blank" rel="noopener noreferrer">
                            <img alt="Meilisearch's logo" fetchpriority="high" width="162" height="25" decoding="async"
                                 data-nimg="1"
                    ...
                    <ul class="DocSearch-Commands">
                        ...
                        <li>
                            ...
                            </kbd><kbd class="DocSearch-Commands-Key"><svg width="15" height="15" aria-label="Arrow up"
                                                                           role="img">
                                    <g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                       stroke-width="1.2">
