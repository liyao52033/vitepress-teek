...
description: '文章介绍了 Vitepress 使用 markdown-it 来解析和渲染 Markdown，支持多种语法扩展和插件。Teeker 提供了多种实用的 Markdown 插件，如待办列表、内容居中/居右容器及多种卡片容器，用户可以通过简单的 HTML 和 CSS 定制丰富的页面功能和样式，极大地增强了 Markdown 的表现力和交互性。'
tags:
  - 插件
author:
  name: 华总
