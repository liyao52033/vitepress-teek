...
description: '本文介绍了vitepress-plugin-sidebar-depth插件的功能、安装使用方法及详细配置选项。该插件能自动扫描项目中的markdown文件，生成结构清晰的侧边栏，支持多语言和文件排序，且提供丰富的配置和钩子函数，方便用户根据需求定制侧边栏表现。使用该插件可以提升vitepress文档站点的维护效率和用户体验。'
tags:
  - 插件
author:
  name: 华总
