...
description: '一个用于 VitePress 的插件，能自动为 Markdown 文件生成标题和日期等 frontmatter，支持用户自定义扩展内容。通过简单安装和配置即可使用，并能结合 vitepress-plugin-catalogue 插件添加永久链接和分类标签。该插件仅在项目启动时生效，有效提升文档管理的自动化和规范化水平。'
tags:
  - 插件
categories:
  - 工具类
